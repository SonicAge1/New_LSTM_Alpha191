...
def train1(epoch_num, criterion, optimizer, module, data_loaders, test_loader, jump):
    ...
    for epoch in range(epoch_num):
        ...
        if pre_min_loss == min_loss:
            early_stop_counter += 1
            if early_stop_counter == 10:
                save_and_plot_results(50, module, train_loss_plot, test_loss_plot, batch_size, learning_rate, hidden_size, num_layers, dropout, min_loss, 'test_loss超过十轮未下降,早停')
                break  # 早停
        else:
            early_stop_counter = 0

        pre_min_loss = min_loss
        # print(f'Epoch {epoch + 1}: Testing loss = {test_loss:.4f}')

        if (epoch + 1) % 10 == 0:
            if (sum(train_loss_plot) / (epoch + 1)) > 6.5:
                save_and_plot_results(0, module, train_loss_plot, test_loss_plot, batch_size, learning_rate, hidden_size, num_layers, dropout, min_loss, '损失值不下降,早停')
                break  # 早停 
...
def save_and_plot_results(epoch, module, train_loss_plot, test_loss_plot, batch_size, learning_rate, hidden_size, num_layers, dropout, min_loss, msg):
    ...
    plt.legend()
    plt.savefig(f'./img/epoch:{epoch+1}_min_test_loss:{min_loss:.3f}_{batch_size}bs_{learning_rate}lr_{hidden_size}hs_{num_layers}nl_{dropout}dr.png')
    send_email_with_attachment(f"./img/epoch:{epoch+1}_min_test_loss:{min_loss:.3f}_{batch_size}bs_{learning_rate}lr_{hidden_size}hs_{num_layers}nl_{dropout}dr.png",
                                "自动通知", f'epoch:{epoch+1}_min_test_loss:{min_loss:.3f}_{batch_size}b_{learning_rate}lr_{hidden_size}hs_{num_layers}nl_{dropout}do, {msg}')
    plt.close()
...
# else:
#     module = Net(input_size, hidden_size, num_layers, output_size, batch_size, seq_length, dropout)  # hidden_size,num_layers,batch_size is var
#     if torch.cuda.device_count() > 1:
#         module.to('cuda')
#         module = nn.DataParallel(module)
...
if trainBool:
    batch_size_r = [512, 128, 1024, 256, 64]
    learning_rate_r = [0.001, 0.0005, 0.01]
    hidden_size_r = [256, 512]
    num_layers_r = [2, 3, 4]
    dropout_r = [0.1, 0.2, 0.3, 0.4]
    jump = 0
    for batch_size in batch_size_r:
        for learning_rate in learning_rate_r:
            for hidden_size in hidden_size_r:
                for num_layers in num_layers_r:
                    for dropout in dropout_r:
                        if jump < 355:
                            jump += 1
                        else:
                            jump += 1
                            print(f"Config: batch_size={batch_size}, learning_rate={learning_rate}, hidden_size={hidden_size}, num_layers={num_layers}, dropout={dropout}")
                            ...
                            train1(epoch_num, criterion, optimizer, module, data_loaders, test_loader, jump)
